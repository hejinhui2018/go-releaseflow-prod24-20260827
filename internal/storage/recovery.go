...
func (s *FileStore) Packet(packetID string) (model.Packet, error) {
	if packet, ok, err := s.snapshotAtOrAfterJournal(packetID); err != nil {
...
func (s *FileStore) snapshotAtOrAfterJournal(packetID string) (model.Packet, bool, error) {
	snapshot, err := s.LoadSnapshot(packetID)
	...
	if len(events) == 0 || snapshot.Revision >= events[len(events)-1].Revision {
